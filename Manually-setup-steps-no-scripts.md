...
NB: I haven't tested the manual steps myslef but extracted the followings from my automated provisioning script using AI, if theres any issue please leave comments and i will check, best recommendation is use [dynamic provisioning script](unifi-pxe-setup.sh), which require zero or very small changes  for preparing PXE server.
